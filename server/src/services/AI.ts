...
export async function generateJudgmentSummary(
  texts: Array<{ type: string; content: string }>,
  date?: string
): Promise<any> {
...
- "bench"`;
  ...
  const textContent = texts
    .map((t, index) => `Text ${index + 1} (${t.type}):\n${t.content}`)
    .join("\n\n");

  const dateInfo = date ? `Judgment Date: ${date}\n\n` : "";
